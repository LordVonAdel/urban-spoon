...
Lobby.prototype.addClient = function(client){ //Add a client to the lobby
  if (this.game != null){
...
Lobby.prototype.removeClient = function(client){ //Removes a client from the lobby
...
Lobby.prototype.setInfo = function(text){ //gives the status of the lobby to all players. Errors and things
  this.broadcast('lobbyStatus',text);
}

Lobby.prototype.reportIssues = function(){
  this.checkReady();
...
Lobby.prototype.sync = function(){ //Synchronize the lobby between all players in the lobby
  for(var i=0; i<this.clients.length; i++){
    this.clients[i].socket.emit("lobby",this.getSync());
  }
}

Lobby.prototype.getSync = function(){ //get the data needed for the clients
...
Lobby.prototype.broadcast = function(msg,data){ //sends a message to everyone in the lobby
  for(var i=0; i<this.clients.length; i++){
    this.clients[i].socket.emit(msg,data);
  }
}

Lobby.prototype.broadcastTeam = function(msg,data,team){ //sends a message to everyone in the lobby in specific team
  for(var i=0; i<this.clients.length; i++){
...
Lobby.prototype.checkTeams = function(){ //refreshes the teams array
  this.teams = [];
  for(var i=0; i<this.settings.teamNumber; i++){
...
Lobby.prototype.getTeam = function(teamIndex){
  return this.teams[teamIndex];
}

Lobby.prototype.second = function(){ //called every second
  if (this.countdown != Infinity){
    this.countdown --;
...
Lobby.prototype.tick = function(){ //called every tick from the tickloop in app.js
  if (this.game != null){
    this.game.tick();
  }
}

Lobby.prototype.startGame = function(){ //starts the game
...
Lobby.prototype.changeSettings = function(data){ //when the host change settings
...
Lobby.prototype.changeTeam = function(client,team){ //change the team of a player
  if (this.game == null){
